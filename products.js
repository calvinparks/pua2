    ...
    let ready = () => {
        let removeItemFromCartButtons = document.getElementsByClassName('btn-remove-item')
        ...
        document.getElementsByClassName('btn-purchase')[0].addEventListener('click', orderNowClicked)
    ...
    addItemToCartClicked = (event) =>{
        ...
        let imageSrc = "";
        addItemToCart(title, price, imageSrc, qty)
        updateTheCartTotal()
    }
    ...
    addItemToCart = (title, price, imageSrc, qty) => {
        ...
        // check to see if the item is already in the cart if so then reject
    ...
    let cartRowContents = `
        ...
        cartRow.getElementsByClassName('btn-remove-item')[0].addEventListener('click', removeItemFromCart)
